...
![Setting up a Data View using the Web App Index](images/02-create-data-view-webapp-data.png)

The created webapp data view. 

![Setting up a Data View using the Web App Index](images/03-created-data-view-webapp-data.png)

Let's go the Discover App and check for data. 

![Checking the data in Discover App](images/04-go-to-discover-app-to-view-data.png)

Then we need to select the WebApp index pattern 

![Select the Webapp index](images/05-select-webapp-index-pattern.png)

Since this is not a realtime data feed, but a historical data set. We need to adjust the time range. 

![Adjust the time range correctly to view the webapp data](images/06-adjust-the-time-range.png) 

We can now see the webapp data. 

![We can now see the web app data](images/07-we-can-see-webapp-data.png)

* Step 2 - Enabling a Trial 30 License to use Elasticsearch Machine Learning features

![Let's go to machine learing app in the kibana main menu](images/08-select-machine-learning-app-kibana-main-menu.png)
...
![Enable Trial License](images/09-we-have-to-enable-trial.png)

![Start Trial](images/10-start-trial.png)

Now we should see that the Trial Key is activated. 

![Trial Key Activiated](images/11-trial-activated.png)

Now let us go back to machine learning app. 

![Let's go back to machine learning app](images/12-select-machine-learning-app-again.png) 
...
![Let's create a machine learning job](images/13-create-anomaly-detection-job.png)

We need to select a data view to specify which data set is to be used by the machine learning job

![Select a data view](images/14-select-a-data-view.png)

Let us start with a single metric anomaly detection job. 

![Select Single Metric Job option](images/15-select-single-metric-anomaly-option.png)
...
![Select Time range of the data set](images/16-select-time-range-full-data-set.png)
...
![Select COUNT(Event Rate)](images/17-choose-event-rate-count-as-the-field.png)
...
![Select 1m as bucket size](images/18-choose-1m-as-bucket-span.png)

Then we need to give a name to the machine learning job. 

![Give a name to the machine learning job](images/19-give-a-name-for-the-ml-job.png)

Validating the job configuration. 

![Validating the ML Job Configuration](images/20-validate-job-settings.png)

Now, we can create the job. 

![Create the Job](images/21-select-create-job.png)

After few seconds or minutes (depending your elasticsearch node configuration) the job should finish. 

![finished ML job](images/22-we-can-see-the-model-training-is-completed-select-view-result.png)
...
![View Results of the ML Job](images/23-adjust-time-slider-to-focus-on-anomalous-transactions.png)
...
![Forecasting Event Rate behavior for the next 7 days](images/24-forecasting-7days.png)

Forecast results

![forecast results](images/25-forecast.png)
...
![Create another machine learning job](images/26-create-another-anomalous-job-for-data-transwer-volumes.png)
...
![Select the Webapp Data View](images/27-select-webapp-data-view.png)

Select Multi Metric option

![Select Multi Metric Option](images/28-select-multi-metric.png)

As done previously, let's select the time range. 

![Select Time Range](images/29-use-full-data-set.png)

* Configuring the Multi Metric ML Job

Let's select the *MEAN(http.response.bytes)* as the metric

![Choose MEAN(http.response.bytes)](images/30-choose-mean-response-bytes.png)

Further configure the job

![More configurationgs](images/31-configure-analysis.png)

Give a name to the ML job

![Give a name to the ML Job](images/32-give-a-name-to-the-ml-job.png)

Validate the ML job

![Validate the Job](images/33-validate-job.png)

Create the ML Job

![Create the ML Job](images/34-create-job.png)

After few seconds, the Job should finishe successfully. 

![Job Finished, Select View Results](images/35-select-view-results.png)

We can clearly see some anomalies in the MEAN of *http.response.bytes*

![Amonalies found](images/36-anomalies-detected.png)

Details of the anomalous events

![Details of the anomalous events](images/37-digging-into-details.png)
...
![Population Analysis](images/38-population-analysis.png)

As previous, selecting the time rage of the data set. 

![Selecting the time range of the data set](images/39-select-full-data-set.png)

Configuring the ML Job

![Configuring the ML Job](images/40-configure-ml-job.png)

Give a name to the ML Job

![Give a name to the ML Job](images/41-give-a-name-to-ml-job.png)

Create the JOB

![Create the ML Job](images/42-create-ml-job.png)

Let us now view the results. 

![View Results](images/43-view-results.png)

Summarizing the Results for the Source IP addresses

![Summary of Results for the Source IP addresses](images/44-results-summary-for-source-ip.png)

Further details about the anomalous Source IP address. 

![Further details about the Source IP](images/45-more-details-about-anomalous-source-ip.png)
...
![Elastic Discover App](images/46-discover-app-analysis.png)

We can configure what information we want to see from the actual Event details. 

![Customize the Actual Event Details screen](images/47-custom-detail-view-of-the-hack.png)
...
![Create another ML Job](images/48-create-another-new-data-view-webapp-tagged-data.png)

Data view created

![Data View created](images/49-new-data-view-webapp-tagged.png)
...
![Select Data Frame Analysis](images/50-select-data-frame-analysis-create-job.png)
...
![Select Data View](images/51-select-webapp-tagged-data-view.png)

Select *Classification*

![Select Classification](images/52-select-classification.png)

* Let us now configure the *Classification ML Job*

![Configure Classification ML Job](images/53-configure-classification-ml-job.png)
...
![Configure Classification ML Job](images/54-configure-classification-ml-job.png)

No additional parameters for this use case

![No Additional Parameters](images/55-no-additional-parameters.png)

Give a name to the classification ML Job

![Give a name to the classification ML job](images/56-give-a-name-classification-ml-job.png)

Validate the Job

![Validate the Job](images/57-validation.png)

Create *Classification ML Job*

![Create classification ML Job](images/58-create-classification-ml-job.png)

Let's now go back to the data frame analsys to view the results. 

![Go back to data frame analysis](images/59-go-back-to-data-frame-analytics.png)

Classification Job finished execution

![finished execution](images/60-classification-job-finished.png)

Let's look at the results 

![Let's look at the results](images/61-results-explorer.png)

The trained models section. 

![Trained Model Section](images/62-trained-models.png)
